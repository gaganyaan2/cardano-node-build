...
RUN apt update -y && \
    apt install libnuma-dev automake build-essential pkg-config libffi-dev libgmp-dev libssl-dev libtinfo-dev libsystemd-dev zlib1g-dev make g++ tmux git jq wget libncursesw5 libtool autoconf liblmdb-dev curl -y && \
    apt clean -y && \
...
RUN apt update -y && apt install llvm -y
RUN export PATH="$PATH:/root/.ghcup/bin/"; cabal update && cabal build all --disable-tests
...
ARG DEBIAN_FRONTEND=noninteractive
RUN apt update -y && \
    apt install libnuma-dev build-essential libssl-dev libsodium-dev -y && \
    apt clean -y && \
    rm -rf /var/lib/apt/lists/*
RUN echo 'export LD_LIBRARY_PATH="/usr/local/lib:$LD_LIBRARY_PATH"' >> ~/.bashrc
RUN echo 'export PKG_CONFIG_PATH="/usr/local/lib/pkgconfig:$PKG_CONFIG_PATH"' >> ~/.bashrc
